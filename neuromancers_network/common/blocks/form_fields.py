...
from .content import Grid
from .content import InlineBlock
from .content import InlineRichTextBlock
from .content import InlineTextBlock
from .content import Row


class FieldLabelBlock(InlineTextBlock):
    """
    A block representing a form field label, with optional styling.
    """
...
class FieldHelpTextBlock(InlineRichTextBlock):
    """
    A block representing form field help text, with optional styling.
    """

    class Meta:
        icon = "help"
        group = _("Field Help Text")
        collapsed = True


class FieldThemeBlock(InlineBlock):
    """
    A block representing form field theme settings, allowing customization of
    form field appearance on a per-field basis when used within a flexible form
    """
...
class HiddenFieldBlock(OptionalFormFieldBlock):
    field_class = forms.CharField
...
class FormFieldsBlock(StreamBlock):
    ...
    additional_content = ContentBlock(group=_("Additional Content"))
...
class FormRowBlock(StructBlock, Row):
    fields = FormFieldsBlock()
...
class FormColumnBlock(StructBlock, Column):
    fields = FormFieldsBlock()
...
class FormGridBlock(StructBlock, Grid):
    rows = ListBlock(FormRowBlock())
...
class FormLayoutBlock(FormFieldsBlock):
    row = FormRowBlock()
    column = FormColumnBlock()
    grid = FormGridBlock()
...
class FormBlock(StreamBlock):
    fields = FormFieldsBlock()
    layout = FormLayoutBlock()
    steps = FormStepsBlock()
    to_email = CharBlock(
        required=False,
    ...
    success_url = LinkBlock(
        required=True,
        help_text="Use this if you want users to be directed to another page after submission. Overrides success content and hide on success",
    )
    hide_on_success = BooleanBlock(
        help_text="Hide the form after a successful submission",
        default=True,
    )
    success_content = ContentBlock(
        help_text=(
            "Use this if you want users to stay on the same page after submission and see new content, i.e 'Your form has been submitted!'",
            "There must NOT be a success URL.",
        ),
    )
...
class ContentFormBlock(StreamBlock):
    form = FormBlock()
