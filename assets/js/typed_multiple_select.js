...
var toggleSelectedOptions = (button) => {
  ...
  Array.from(select.querySelectorAll("option[selected]")).map((opt) => {
    const _button = createSelectedButton(
      toggleSelectedOptions,
      opt.textContent,
      select.dataset.removeTitle,
    );
      ...
      group
        ...
        .forEach((checkItem) =>
          checkItem.classList.toggle(
            "hidden",
            !matchingArr.some((item) => item.id === checkItem.id),
          ),
        );
    } else {
      group.classList.add("hidden");
    ...
    try {
      groups.forEach((group) => {
        ...
        const visibleItems = Array.from(
          group.querySelectorAll("li:not(.hidden)"),
        );
...
const initializeInput = (inputEl) => {
  ...
  const groups = wrapperEl.querySelectorAll(
    ".typed_select_multiple__options_group",
  );
...
(function () {
  document.addEventListener("DOMContentLoaded", () => {
    document
      .querySelectorAll('[data-id="typed_select_multiple__input"]')
      .forEach((inputEl) => {
