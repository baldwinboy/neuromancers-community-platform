import uuid
from django.contrib.auth import get_user_model
from django.core.validators import MinValueValidator, MaxValueValidator
from django.db import models
from django.utils.translation import gettext as _

from guardian.models import GroupObjectPermissionBase, UserObjectPermissionBase

from apps.events.models import FocusArea, LivedExperience
from apps.events.validators import validate_language_codes
from apps.events.choices import (
    currency_choices,
)

User = get_user_model()


class NotificationChoices(models.IntegerChoices):
    """
    Notifications may sent on the platform, via email, or not at all
    """

    NONE = 0, _("None")
    WEB_ONLY = 1, _("On this site")
    EMAIL = 2, _("Via email")
    ALL = 3, _("All")

class NotificationSubjectChoices(models.IntegerChoices):
    ACCOUNT = 0, _("Account")
    PAYMENT = 1, _("Payment")
    SESSION = 2, _("Session")
    REMINDER = 3, _("Reminder")

class Notifications(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    sent_at = models.DateTimeField(auto_now=True)
    sent_to = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name="notifications",
    )
    subject = models.IntegerField(
        choices=NotificationSubjectChoices,
    )
    body = models.TextField(max_length=10_240)
    read = models.BooleanField(default=False)

# Certified training managed by admins
class CertifiedTraining(models.Model):
    certified_user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name="certified_training",
        primary_key=True,
    )
    issuing_user = models.ForeignKey(
        User,
        on_delete=models.DO_NOTHING,
        related_name="issued_certified_training",
        null=True,
        blank=True,
    )
    issued_at = models.DateTimeField()
    expires_at = models.DateTimeField(null=True, blank=True)


class CertifiedTrainingUserObjectPermission(UserObjectPermissionBase):
    content_object = models.ForeignKey(CertifiedTraining, on_delete=models.CASCADE)


class CertifiedTrainingGroupObjectPermission(GroupObjectPermissionBase):
    content_object = models.ForeignKey(CertifiedTraining, on_delete=models.CASCADE)


# Notification settings for all users
class BaseNotificationSettings(models.Model):
    requested_session = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when you request a session"),
    )
    responded_session = models.IntegerField(
        choices=NotificationChoices,
        help_text=_(
            "You'll receive a notification when a Care Provider has responded to your request"
        ),
    )
    cancelled_session = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when your session is cancelled"),
    )
    session_reminders = models.IntegerField(
        choices=NotificationChoices,
        help_text=_(
            "You'll receive a notification before one (1) day and/or one (1) hour before your session starts"
        ),
    )
    account_deleted = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when your account is deleted"),
    )
    payment_made = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when you make a payment"),
    )
    payment_refunded = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when your payment is refunded"),
    )

    class Meta:
        abstract = True


class NotificationSettings(BaseNotificationSettings):
    user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name="notification_settings",
        primary_key=True,
    )


class NotificationSettingsUserObjectPermission(UserObjectPermissionBase):
    content_object = models.ForeignKey(NotificationSettings, on_delete=models.CASCADE)


class NotificationSettingsGroupObjectPermission(GroupObjectPermissionBase):
    content_object = models.ForeignKey(NotificationSettings, on_delete=models.CASCADE)


# Notifications for Peer users
class PeerNotificationSettings(BaseNotificationSettings):
    published_session = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when you publish a session"),
    )
    host_session_requested = models.IntegerField(
        choices=NotificationChoices,
        help_text=_(
            "You'll receive a notification when a Care Seeker requests a session from you"
        ),
    )
    host_session_booked = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when you accept a request"),
    )
    host_session_cancelled = models.IntegerField(
        choices=NotificationChoices,
        help_text=_(
            "You'll receive a notification when you cancel a session or a Care seeker  "
        ),
    )
    host_session_reminders = models.IntegerField(
        choices=NotificationChoices,
        help_text=_(
            "You'll receive a notification one (1) day and/or one (1) hour before a session provided by you starts"
        ),
    )
    payment_received = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when you receive a payment"),
    )
    payment_refund_request = models.IntegerField(
        choices=NotificationChoices,
        help_text=_(
            "You'll receive a notification when a Care Seeker requests a refund from you"
        ),
    )
    payment_refunded = models.IntegerField(
        choices=NotificationChoices,
        help_text=_("You'll receive a notification when you refund a payment"),
    )

    user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name="peer_notification_settings",
        primary_key=True,
    )


class PeerNotificationSettingsUserObjectPermission(UserObjectPermissionBase):
    content_object = models.ForeignKey(
        PeerNotificationSettings, on_delete=models.CASCADE
    )


class PeerNotificationSettingsGroupObjectPermission(GroupObjectPermissionBase):
    content_object = models.ForeignKey(
        PeerNotificationSettings, on_delete=models.CASCADE
    )


# Access needs for users
class AccessNeeds(models.Model):
    description = models.TextField(
        max_length=10_240,
        null=True,
        blank=True,
        help_text=_("You can override these when requesting a session"),
    )
    user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name="access_needs",
        primary_key=True,
    )


class AccessNeedsUserObjectPermission(UserObjectPermissionBase):
    content_object = models.ForeignKey(AccessNeeds, on_delete=models.CASCADE)


class AccessNeedsGroupObjectPermission(GroupObjectPermissionBase):
    content_object = models.ForeignKey(AccessNeeds, on_delete=models.CASCADE)


# Settings for Peer sessions per user
class PeerSessionSettings(models.Model):
    stripe_link = models.URLField(
        help_text="Provide a link to an existing Stripe account. You won't be able to receive payments without this."
    )
    languages = models.CharField(
        help_text=_(
            "These are languages that you provide sessions in. You can override these when creating or editing a session."
        ),
        max_length=620,
        validators=[validate_language_codes],
    )

    lived_experiences = models.ManyToManyField(LivedExperience)
    focus_areas = models.ManyToManyField(FocusArea)

    currency = models.CharField(
        max_length=3,
        choices=currency_choices,
        default="GBP",
        help_text=_(
            "Default currency for sessions. You can override this when creating or editing a session."
        ),
    )
    price = models.IntegerField(
        default=0,
        validators=[
            MinValueValidator(0, message=_("Cannot charge negative values")),
            MaxValueValidator(
                9_999_999,
                message=_(
                    "Cannot charge over 99,999 of a currency's unit. For higher values, try a different currency."
                ),
            ),
        ],
        help_text=_(
            "Default price for sessions. You can override this when creating or editing a session."
        ),
    )
    concessionary_price = models.IntegerField(
        validators=[
            MinValueValidator(0, message=_("Cannot charge negative values")),
            MaxValueValidator(
                9_999_999,
                message=_(
                    "Cannot charge over 99,999 of a currency's unit. For higher values, try a different currency."
                ),
            ),
        ],
        help_text=_(
            "Default concessionary price for sessions. You can override this when creating or editing a session."
        ),
        null=True,
        blank=True,
    )
    allow_custom_price = models.BooleanField(
        help_text=_(
            "Enable custom pricing by default. You can override this when creating or editing a session."
        ),
        default=True,
    )

    user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name="peer_session_settings",
        primary_key=True,
    )


class PeerSessionSettingsUserObjectPermission(UserObjectPermissionBase):
    content_object = models.ForeignKey(PeerSessionSettings, on_delete=models.CASCADE)


class PeerSessionSettingsGroupObjectPermission(GroupObjectPermissionBase):
    content_object = models.ForeignKey(PeerSessionSettings, on_delete=models.CASCADE)
